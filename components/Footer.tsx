    ...
    <footer className="relative border-t border-[#1F1F1F] bg-[#0A0A0A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="md:col-span-2">
            ...
            <p className="text-sm text-[#444] leading-relaxed max-w-xs">
              {SITE.tagline} — AI-powered websites and automation systems that work 24/7 so your business never misses a lead.
            </p>
            <p className="text-xs text-[#333] mt-4">{SITE.location}</p>
          </div>

          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#333] mb-4">Navigation</p>
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#333] mb-4">Industries</p>
            <ul className="space-y-2.5">
              {FOOTER_INDUSTRIES.map((ind) => (
                <li key={ind} className="text-sm text-[#444]">{ind}</li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="mt-12 pt-6 border-t border-[#1F1F1F] flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-[#333]">© 2025 Flowtix. All rights reserved.</p>
          <p className="text-xs text-[#333]">Built for businesses that want to grow faster.</p>
        </div>
      </div>
    </footer>
  );
